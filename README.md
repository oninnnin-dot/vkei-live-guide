...
現場情報は公式で探す。遠征と会場準備の不安はここで減らす。

「推し活遠征ナビ」は、ライブ、アイドル現場、アニメ・漫画系イベント、舞台、リリースイベントなどへ行く前に確認したい会場周辺情報と遠征準備を整理する静的Webサイトです。公演情報やチケット情報を集めるサイトではなく、ロッカー、クローク、荷物、待機場所、雨の日、終演後の帰り方、遠征費の判断を補助します。

元バンドマンの管理人が、独断と偏見も交えながら各地のライブハウスや小規模現場の情報を随時追加し、推し活遠征をサポートしていくサイトです。
...
npm run check:public-text
```

ビルド成果物は `dist/` に出力されます。`check:public-text` は公開HTMLに出したくない語が混ざっていないか確認します。
...
ライブハウス経験者目線の「ここで困りやすい」を、アイドル、アニメ・漫画、舞台、リリースイベントなどの推し活全般でも使いやすい情報に置き換えて蓄積します。
...
旧SEO URLは互換性のため残しつつ、表示文言は推し活全般向けにしています。

## 会場データ

会場データは `data/venues.json` で管理します。今回のリブランドでは、データ構造は維持し、画面表示側で推し活向けのラベルに変換しています。
...
個人ブログ、note、体験レポ、Q&A、SNS由来の情報は、文章コピーせず要点だけ独自表現に変換します。
...
- 公開ページでは `sourceLinks` だけを出典として表示する
...
一部のガイドやチェックリストに、遠征準備に役立つ商品リンクを掲載します。モバイルバッテリー、雨具、圧縮袋、サブバッグ、アクスタ用ケース、ライトスティック用ポーチ、遠征グッズ、交通・宿泊サービスなどを想定しています。

会場情報や当日の動き方を優先し、成人向けの商品やサービスは扱いません。

## デザイン方針

黒を基調に、アンティークゴールドや深い赤紫を差し色にした、劇場パンフレット風の落ち着いたUIです。
...
特定の出演者、作品、公式ロゴ、画像、歌詞、衣装、アートワーク、固有デザインは使用しません。
